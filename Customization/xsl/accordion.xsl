   ...
   <xsl:template match="*[contains(@class,' topic/bodydiv ') and contains(@outputclass, 'accordion')]">
      <div>
         ...
         <!--xsl:call-template name="commonattributes">
            <xsl:with-param name="default-output-class" select="'accordion'"/>
         </xsl:call-template-->
      ...
      <div class="accordion-item">
         ...
         <xsl:element name="{$headLevel}">
            ...
            <button class="accordion-button" type="button" data-bs-toggle="collapse" >
               <xsl:attribute name="aria-expanded" select="(count(preceding-sibling::*[contains(@class, ' topic/section ')]) = 0)"/>
               <xsl:attribute name="class">
