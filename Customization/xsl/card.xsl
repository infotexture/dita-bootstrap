  ...
  <xsl:template match="*" mode="card">
    <xsl:param name="headLevel">
      ...
      <xsl:choose>
        ...
        <xsl:when test="count(preceding-sibling::*[contains(@class, ' topic/title ')]) > 0">h<xsl:value-of select="$headCount+1"/></xsl:when>
    ...
    <xsl:variable name="bootstrap-class">
    	<xsl:choose>
        <xsl:when test="count(preceding-sibling::*[contains(@class, ' topic/title ')]) > 0">sectiontitle card-subtitle text-muted</xsl:when>
